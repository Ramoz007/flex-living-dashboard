...
const openDatabase = (store: IndexDatabaseStore): Promise<IDBDatabase> =>
  new Promise((resolve, reject) => {
    const request = indexedDB.open(VARIABLES.indexDatabaseName, VARIABLES.indexDatabaseVersion);
 ...
 */
export const clearIndexedDatabase = async (store: IndexDatabaseStore): Promise<void> => {
  const db = await openDatabase(store);
  const transaction = db.transaction(store, "readwrite");
  transaction.oncomplete = (): void => {
    db.close();
